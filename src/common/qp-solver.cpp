...
void QPSolver::reorderSolution(VectorXd & qpSolution, VectorXi & constraints,
             VectorXi & initialConstraints, Eigen::VectorXd & initialLagrangeMultiplier,
             Eigen::VectorXd & lagrangeMultiplier){
  VectorXd solutionTmp = qpSolution;
  VectorXi constraintsTmp = constraints;
  VectorXd lagrangeMultiplierTmp = lagrangeMultiplier;

  for (int i=0;i<nbVar_;++i) {
    qpSolution(i) = solutionTmp(varOrder_(i));
    constraints(i) = constraintsTmp(varOrder_(i));
    lagrangeMultiplier(i) = lagrangeMultiplierTmp(varOrder_(i));
  }
  for(int i=0;i<nbCtr_;++i) {
    constraints(i+nbVar_) = constraintsTmp(ctrOrder_(i+nbVar_));
    lagrangeMultiplier(i+nbVar_) = lagrangeMultiplierTmp(ctrOrder_(i+nbVar_));
  }

  initialConstraints= constraints;
  initialLagrangeMultiplier = lagrangeMultiplier;
}
