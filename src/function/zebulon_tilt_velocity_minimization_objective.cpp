...
TiltVelMinimizationObjective<Scalar>::TiltVelMinimizationObjective(const LIPModel<Scalar>& lipModel,
...
template <typename Scalar>
TiltVelMinimizationObjective<Scalar>::~TiltVelMinimizationObjective(){}

template <typename Scalar>
const typename
Type<Scalar>::MatrixX& TiltVelMinimizationObjective<Scalar>::getGradient(const VectorX& x0)
{
...
template <typename Scalar>
const typename Type<Scalar>::MatrixX& TiltVelMinimizationObjective<Scalar>::getHessian()
{
  assert(baseModel_.getNbSamples() == lipModel_.getNbSamples());
  assert(baseModel_.getSamplingPeriod() == lipModel_.getSamplingPeriod());
...
template <typename Scalar>
void TiltVelMinimizationObjective<Scalar>::updateTiltContactPoint()
{
  assert(baseModel_.getNbSamples() == lipModel_.getNbSamples());
  assert(baseModel_.getSamplingPeriod() == lipModel_.getSamplingPeriod());
...
template <typename Scalar>
void TiltVelMinimizationObjective<Scalar>::computeConstantPart()
{
  assert(baseModel_.getNbSamples() == lipModel_.getNbSamples());
  assert(baseModel_.getSamplingPeriod() == lipModel_.getSamplingPeriod());
  ...
  const LinearDynamic<Scalar>& tiltVelAngleDyn = baseModel_.getBaseTiltAngularVelLinearDynamic();
  ...
  MatrixX U = (A1*tiltAngleDyn.U*tiltVelAngleDyn.Uinv
             - A6*tiltAngleDyn.U*tiltVelAngleDyn.Uinv*tiltAngleDyn.Uinv);
  ...
  dynPsiX_.S = uInv_*A6*angularAccRelativeToAngleDyn.S
             - tiltVelAngleDyn.U*tiltAngleDyn.Uinv*tiltAngleDyn.S+tiltVelAngleDyn.S;
