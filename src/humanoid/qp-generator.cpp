...
void QPGenerator::precomputeObjective(){
  ...
  int nbUsedWeightings = weighting_->JerkMin.size(); //
  int size = generalData_->nbFeedbackSamplesStandard();
  Qconst_.resize(size*nbUsedWeightings);
  QconstN_.resize(size*nbUsedWeightings);
  choleskyConst_.resize(size*nbUsedWeightings);
  pconstCoM_.resize(size*nbUsedWeightings);
  pconstVc_.resize(size*nbUsedWeightings);
  pconstRef_.resize(size*nbUsedWeightings);
  ...
  for (int i = 0; i < nbUsedWeightings; ++i) {
    for (double s = generalData_->MPCSamplingPeriod;
      ...
      const LinearDynamics & CoPDynamics = robot_->body(COM)->dynamics(copDynamicX);
      ...
      G = weighting_->instantVelocity[i]*tmpMat_ + weighting_->JerkMin[i]*tmpMat2_;
      Qconst_[nb] = G;

      QconstN_[nb] = G + weighting_->CopCentering[i]*MatrixXd::Identity(N,N);
      ...
      pconstCoM_[nb] = CoPDynamics.UInvT*VelDynamics.UT*weighting_->instantVelocity[i]*pondFactor*pconstCoM_[nb];
      pconstCoM_[nb]-= CoPDynamics.UInvT*weighting_->JerkMin[i]*pondFactor*CoPDynamics.UInv*CoPDynamics.S;

      pconstVc_[nb]  = CoPDynamics.UInvT*weighting_->JerkMin[i]*pondFactor*CoPDynamics.UInv;
      pconstVc_[nb] += CoPDynamics.UInvT*VelDynamics.UT*weighting_->instantVelocity[i]*pondFactor*VelDynamics.U*CoPDynamics.UInv;

      pconstRef_[nb] = -CoPDynamics.UInvT*VelDynamics.UT*weighting_->instantVelocity[i]*pondFactor;
    }
  }
...
void QPGenerator::buildObjective(const MPCSolution & result) {
  ...
  precomputedMatrixNumber += weighting_->activeWeighting * generalData_->nbFeedbackSamplesStandard();
...
void QPGenerator::buildConstraints(const MPCSolution & result){
  ...
  if (nbStepsPreviewed>0){ //Check if there are remaining steps
...
void QPGenerator::convertCopToJerk(MPCSolution & result){
  ...
  const MatrixXd & rot = preview_->rotationMatrix();
  const BodyState & CoM = robot_->body(COM)->state();
  const LinearDynamics & CoP = robot_->body(COM)->dynamics(copDynamicX);
...
void QPGenerator::buildInequalitiesFeet(const MPCSolution & result){
  ...
  //DX, DY and DC are filled with "0":
  ...
  for( int i=0; i<generalData_->nbSamplesQP; ++i ){
    ...
    if( prwSS_it->stateChanged && prwSS_it->stepNumber>0 && prwSS_it->phase != DS){
      ...
      feetInequalities_.DX.block( stepNumber*nbIneq, stepNumber, nbIneq, 1)
          = hull.A.segment(0, nbIneq);
      feetInequalities_.DY.block( stepNumber*nbIneq, stepNumber, nbIneq, 1)
          = hull.B.segment(0, nbIneq);
