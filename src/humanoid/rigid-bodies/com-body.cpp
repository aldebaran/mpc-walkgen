...
void CoMBody::interpolateTrunkOrientation(MPCSolution &result,
  ...
  interpolation_->computePolynomialNormalisedFactors(factor_, state().yaw, nextTrunkState, T);
  for(int i=0; i < nbSampling; ++i){
    double ti = (i+1)*generalData_->actuationSamplingPeriod;
    interpolation_->selectFactors(subfactor_, factor_, ti, T);
    result.state_vec[0].trunkYaw_(i) = p(subfactor_, ti/T);
    result.state_vec[1].trunkYaw_(i) = dp(subfactor_, ti/T)/T;
    result.state_vec[2].trunkYaw_(i) = ddp(subfactor_, ti/T)/(T*T);
  }
}
