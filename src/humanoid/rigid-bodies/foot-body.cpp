...
void FootBody::computeFootInterpolationByPolynomial(MPCSolution &result, Axis axis, int nbSamples,
  ...
  if (result.supportStates_vec[0].foot==footType_){
    ...
    if (result.supportStates_vec[0].phase == DS){
      ...
      interpolation_->computePolynomialNormalisedFactors(factor_, FootCurrentState,
                                                         nextSupportFootState, T);

      for (int i = 0; i < nbSamples; ++i) {
        double ti = (i+1)*generalData_->actuationSamplingPeriod;
        interpolation_->selectFactors(subfactor_, factor_, ti, T);
        FootTrajState(i) = p(subfactor_, ti/T);
        FootTrajVel(i)   = dp(subfactor_, ti/T)/T;
        FootTrajAcc(i)   = ddp(subfactor_, ti/T)/(T*T);
      }
    }
  }
}
